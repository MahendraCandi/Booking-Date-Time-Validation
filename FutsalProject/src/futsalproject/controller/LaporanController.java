...
public class LaporanController implements Serializable{
    ...
    public void cetakLaporanPenyewaan(Date tglAwal, Date tglAkhir){
        EntityManager em = null;
        try{
            em = emf.createEntityManager();
            em.getTransaction().begin();
            ...
            JasperPrint jprint=JasperFillManager.fillReport ( getClass().getResourceAsStream("/report/LaporanPenyewaan.jasper"), param, connect);
            JasperViewer viewer=new JasperViewer(jprint, false);
            viewer.setFitPageZoomRatio();
            viewer.setVisible(true);
            connect.close();
